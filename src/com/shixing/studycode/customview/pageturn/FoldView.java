...
public class FoldView extends View {
    ...
    private static final float AUTO_AREA_BUTTOM_RIGHT = 3 / 4F,
            AUTO_AREA_BUTTOM_LEFT = 1 / 8F;// 右下角和左侧自滑区域占比
    private static final float AUTO_SLIDE_BL_V = 1 / 25F,
            AUTO_SLIDE_BR_V = 1 / 100F;// 滑动速度占比
    private static final float TEXT_SIZE_NORMAL = 1 / 40F,
            TEXT_SIZE_LARGER = 1 / 20F;// 标准文字尺寸和大号文字尺寸的占比
    ...
    private void computeShortSizeRegion() {
        ...
        // 将Path转化为Region
        mRegionShortSize.setPath(pathShortSize, new Region(
                (int) rectShortSize.left, (int) rectShortSize.top,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!mRegionShortSize.contains((int) mPointX, (int) mPointY)) {
            // 如果不在则通过x坐标强行重算y坐标
            mPointY = (float) (Math.sqrt((Math.pow(mViewWidth, 2) - Math.pow(
                    mPointX, 2))) - mViewHeight);
        ...
        if (sizeLong > mViewHeight) {
            ...
            // 三角形AMN的MN边
            float largerTrianShortSize = an
                    / (sizeLong - (mViewHeight - mPointY))
                    * (mViewWidth - mPointX);
    ...
    private void drawBitmaps(Canvas canvas) {
        ...
        mPageIndex = mPageIndex > mBitmaps.size() ? mBitmaps.size()
                : mPageIndex;
        ...
        // canvas.drawBitmap(mBitmaps.get(end - 1), 0, 0, null);
        canvas.drawBitmapMesh(mBitmaps.get(end - 1), SUB_WIDTH, SUB_HEIGHT,
    ...
    private Region computeRegion(Path path) {
        ...
        region.setPath(path, new Region((int) f.left, (int) f.top,
                (int) f.right, (int) f.bottom));
        return region;
    }
    ...
    private void slide() {
        ...
        if (!isLastPage && isNextPage
                && (mPointX - mAutoSlideV_BL <= -mViewWidth)) {
            mPointX = -mViewWidth;
            mPointY = mViewHeight;
            mPageIndex++;
            invalidate();
         ...
         */
            ...
            // 并根据x坐标的值重新计算y坐标的值
            mPointY = mStart_Y
                    + ((mPointX - mStart_X) * (mViewHeight - mStart_Y))
                    / (mViewWidth - mStart_X);
